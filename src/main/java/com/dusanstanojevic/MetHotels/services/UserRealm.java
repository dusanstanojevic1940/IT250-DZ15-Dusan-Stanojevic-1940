...
import org.tynamo.security.components.User;
...
public class UserRealm extends AuthorizingRealm {
	 ...
	 protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws org.apache.shiro.authc.AuthenticationException {
		 ...
		 Set<String> roles = new HashSet<String>(1);
		 roles.add(user.role.name());
		 return new SimpleAuthenticationInfo(user.ime, user.pass, getName());
	 }
	 
	 private Korisnik checkUser(String username, String email) {
		 try {
			 Korisnik u = (Korisnik) session.createCriteria(Korisnik.class).add(Restrictions.eq("ime", username)).add(Restrictions.eq("pass", email)).uniqueResult();
			 if (u != null) {
				 return u;
			 }
		 	return null;
	 ...
	 private Korisnik findByUsername(String username) {
		 List<Korisnik> users = session.createCriteria(Korisnik.class).add(Restrictions.eq("ime", username)).list();
		 if (users.size() > 0) {
			 return users.get(0);
		 } else {
			 return null;
